...
def train(model, k):
    ...
    checkpoint_directory = "training_checkpoints_medgan_" + model + '_'+k
...
def gen(model, e, n):
    checkpoint_directory = "training_checkpoints_medgan_" + model + '_' + n
    checkpoint_prefix = os.path.join(checkpoint_directory, "ckpt")
    data = np.load('data/train_' + str(model) + '.npy').astype('float32')
    size = data.shape[-1]
    generator = Generator()
    ...
    np.save('syn/medgan_' + model + '_' + n, syn.astype('int8'))
