...
class AE(tf.keras.Model):
    def __init__(self,size):
        ...
        self.decoder = tf.keras.layers.Dense(size)
...
class Discriminator(tf.keras.Model):
    ...
    def call(self, x):
        ...
        x = tf.concat((x,tf.tile(tf.reduce_mean(x,axis=0,keepdims=True),[batch_size,1])),axis=-1)
...
def train(model, k):
    ...
    checkpoint_directory = "training_checkpoints_medbgan_" + model + '_' + k
    ...
    def d_step(real):
        ...
        with tf.GradientTape() as disc_tape:
            synthetic = ae.decode(generator(z, training=False))
    ...
    def g_step():
        ...
        with tf.GradientTape() as gen_tape:
            synthetic = ae.decode(generator(z, training=True))

            fake_output = discriminator(synthetic)

            gen_loss = 0.5 * tf.reduce_mean(tf.square(tf.math.log(fake_output + 1e-12) - tf.math.log(1. - fake_output + 1e-12)))
...
def gen(model, e, n):
    checkpoint_directory = "training_checkpoints_medbgan_" + model + '_' + n
    checkpoint_prefix = os.path.join(checkpoint_directory, "ckpt")
    data = np.load('data/train_' + str(model) + '.npy').astype('float32')
    size = data.shape[-1]
    generator = Generator(size)
    ...
    def g_step():
        ...
        synthetic = ae.decode_test(generator(z, training=False))
        return synthetic
    ...
    np.save('syn/medbgan_' + model + '_' + n, syn.astype('int8'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', type=str, default='train')   # train / gen
    parser.add_argument('--model', type=str, default='uw')    # 数据集名字
    parser.add_argument('--tag', type=str, default='1')        # checkpoint标识
    parser.add_argument('--epoch', type=str, default='999')    # 生成时用到的ckpt编号
    ...
    # 根据模式执行相应的操作
    if args.mode == 'train':
        train(args.model, args.tag)
    elif args.mode == 'gen':
        gen(args.model, args.epoch, args.tag)
    else:
        print(f"未知的模式: {args.mode}，请使用 'train' 或 'gen'")